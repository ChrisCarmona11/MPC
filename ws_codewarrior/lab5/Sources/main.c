...
int canal_int;			// Variable para ir atravesando el regist
int canal;				// Variable para ir atravesando el regist
		...
		if (Flag_interrup == 1) 
		{
			...
			for (canal = 0; canal < 4; canal++) 
			{
				for (muestra = 0; muestra < 128; muestra++) 
				{
					...
					quadratic[canal] 	+=  toVolts(regist[muestra][canal])* toVolts(regist[muestra][canal]);
					if ( muestra == 127)
					{
						Flag_interrup= 0;
					}
			...
			for (canal = 0; canal < 4; canal++) 
			{
				...
				promedio[canal] 	= toVolts((int) (((float) accumulative[canal]) / 128.0));
